...
summary: "TypeScript/JavaScript로 싱글톤 패턴 구현"
date: "Mar 17 2024"
draft: false
tags:
  - Design Pattern
...
싱글톤 인스턴스는 모듈 간 강한 결합을 만들어 의존성이 높아진다. 이로 인해 단위 테스트를 작성할 때 각 테스트마다 인스턴스를 초기화하거나 격리하기가 까다로워 독립적인 테스트가 어렵다. 또한 싱글톤이 전역적으로 사용되면 어떤 모듈이 해당 인스턴스에 의존하는지 파악하기 어려워 코드의 가독성과 유지보수성이 떨어진다.

싱글톤 패턴은 인스턴스가 하나만 존재하도록 강제하기 때문에, 나중에 여러 인스턴스가 필요해질 경우 패턴을 변경하는 것이 어렵다는 확장성의 제약이 있다.
